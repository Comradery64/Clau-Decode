...
def _asst_tokens(id: str, input: int, cache_read: int = 0, cache_write: int = 0) -> Message:
    return Message(
        id=id, session_id="s", role="assistant",
        usage=TokenUsage(
            input_tokens=input,
            output_tokens=5,
...
class TestTipRegistry:
    ...
    def test_sorts_by_severity_error_first(self):
        ...
        warning = Tip(rule_id="r2", severity="warning", title="W", detail="", evidence=[])
...
class TestRepeatedFileReadRule:
    ...
    def test_one_tip_per_offending_file(self):
        from clau_decode.analytics.tips import RepeatedFileReadRule
        msgs = (
            [_read_msg(f"a{i}", "/a.py") for i in range(4)] +
    ...
    def test_custom_threshold(self):
        ...
        assert len(RepeatedFileReadRule(threshold=5).check(

...
def _asst(id: str, parent: str, input: int, output: int) -> Message:
    return Message(
        id=id, session_id="s", role="assistant", parent_id=parent,
...
class TestPromptStatsScanner:
    def test_scans_prompt_totals(self):
        ...
        msgs = [_user("u1"), _asst("a1", "u1", 100, 20),
                _user("u2"), _asst("a2", "u2", 200, 40)]
    ...
    def test_output_stats(self):
        from clau_decode.analytics.stats import PromptStatsScanner
        msgs = [_user("u1"), _asst("a1", "u1", 10, 50),
                _user("u2"), _asst("a2", "u2", 10, 150)]
        result = PromptStatsScanner().scan(msgs)
...
class TestModelUsageScanner:
    def _asst_model(self, id: str, model: str, input: int = 10, output: int = 5) -> Message:
        return Message(
            id=id, session_id="s", role="assistant", model=model,
            usage=TokenUsage(input_tokens=input, output_tokens=output),
        )
...
class TestToolUsageScanner:
    def _msg_with_tools(self, id: str, tools: list[str]) -> Message:
        blocks = [
            ToolUseBlock(id=f"{id}-{i}", name=t, input={})
            for i, t in enumerate(tools)
        ]
        return Message(id=id, session_id="s", role="assistant",

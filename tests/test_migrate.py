...
class TestEncode:
    def test_non_alnum_becomes_dash(self):
        assert migrate.encode("/Volumes/SD/Dev/My App") == "-Volumes-SD-Dev-My-App"
...
class TestRemapKey:
    def test_exact_and_prefixed(self):
        assert migrate.remap_key("/Volumes/SD", "/Volumes/SD", "/new") == "/new"
        assert migrate.remap_key("/Volumes/SD/a", "/Volumes/SD", "/new") == "/new/a"
        assert migrate.remap_key("/other/a", "/Volumes/SD", "/new") == "/other/a"
...
def _make_source(root, *, with_config=True):
    """Build a minimal Claude config tree under ``root`` (a project on /Volumes/SD)."""
    proj = root / "projects" / "-Volumes-SD-Dev-app"
    sub = proj / "sess1" / "subagents"
    sub.mkdir(parents=True)
    cwd = "/Volumes/SD/Dev/app"
    ...
    (root / ".claude.json").write_text(json.dumps({"projects": {cwd: {"trusted": True}}}))
...
class TestMergeEndToEnd:
    def test_dry_run_writes_nothing(self, tmp_path):
        ...
        dest = tmp_path / "dest"
        rc = migrate.run(_args(source=[src], dest_dir=dest,
                               dest_json=tmp_path / "d.json",
                               **{"from": "/Volumes/SD", "to": "/Users/me/Dev"}))
        assert rc == 0
    ...
    def test_apply_relocates_and_rewrites(self, tmp_path):
        ...
        dest = tmp_path / "dest"
        dest_json = tmp_path / "d.json"
        rc = migrate.run(_args(source=[src], dest_dir=dest, dest_json=dest_json,
                               apply=True, i_have_a_backup=True,
                               **{"from": "/Volumes/SD", "to": "/Users/me/Dev"}))
        assert rc == 0
        ...
        stale = [p for p in newdir.rglob("*")
                 if p.is_file() and "/Volumes/SD" in p.read_text()]
        assert stale == []
        # .claude.json project key remapped
        projects = json.loads(dest_json.read_text())["projects"]
        assert "/Users/me/Dev/Dev/app" in projects
        assert not any(k.startswith("/Volumes/SD") for k in projects)

    def test_requires_backup_flag_for_apply(self, tmp_path):
        src, _ = _make_source(tmp_path / "src")
        rc = migrate.run(_args(source=[src], dest_dir=tmp_path / "d",
                               dest_json=tmp_path / "d.json", apply=True))
    ...
    def test_non_destructive_config_conflict_sidecar(self, tmp_path):
        ...
        migrate.run(_args(source=[src], dest_dir=dest, dest_json=tmp_path / "d.json",
                          apply=True, i_have_a_backup=True,
                          **{"from": "/Volumes/SD", "to": "/Users/me/Dev"}))

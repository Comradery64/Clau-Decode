...
3. **Native-view FE wiring** — enable the Native terminal for Codex sessions; the "login required" label is already provider-aware (Phase 3).
4. **Flip caps** — `CodexAdapter.capabilities` → `can_send=True, can_resume=True` (keep `can_fork=False, can_edit=False` until those flows exist).

...
// No view-mode memory and no auto-native: every session switch lands in
// Decoded with the terminal unmounted, so merely reviewing sessions never
// spawns a PTY. A live terminal (and its claude PTY) starts only on explicit
// intent — opening Native/Split, or focusing the composer to type.
...
export default function ChatView() {
  ...
  useEffect(() => {
    setStallError(null);
    setNativeState(null);
    // Reviewing/navigating never spawns a PTY: every switch lands in Decoded
    // and unmounts the terminal (its cleanup blurs the PTY so it dies fast).
    // A PTY starts only on explicit intent — Native/Split or composer focus.
    setNativeHasMounted(false);
    setViewMode("decoded");
  }, [selectedSessionId]);
      ...
      <div ref={splitContainerRef} style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "row" }}>
        ...
        <div
          style={{
            ...
            // Side-by-side. Unmounted (not just hidden) in Native so React
            // does not reconcile a large message list on every pty_native_state
            // re-render — for sessions with thousands of messages that
            // reconcile was causing ~300ms main-thread blocks per update.
            display: "flex",
            flexDirection: "column",
            minWidth: 0,
            minHeight: 0,
